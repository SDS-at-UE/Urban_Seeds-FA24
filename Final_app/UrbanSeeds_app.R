                          ...
                          fluidRow(
                            div(
                              ...
                              wellPanel(
                                ...
                                br(),
                                HTML(
                                  'This app was developed by Madelyn Tarvin, Olivia Oliver, Theresa Stark, Bakhrom Nasurullaev, and Elimdar Idrisov, students at the 
                                <a href="https://www.evansville.edu/">University of Evansville</a>, led by <br> Dr. Omid Khormali.'
                                ),
                                br(),
                                HTML(
...
server <- function(input, output, session) {
  ...
  output$scatter_zip <- renderPlot({
    ...
    ggplot(df, aes(x = month_date)) +
      ...
      geom_line(aes(y = total_order), color = "blue", linewidth = 1) +
      geom_point(aes(y = total_order), color = "blue", size = 2) +
      
      # Line and points for total_price (scaled)
      geom_line(aes(y = total_price / scale_factor), color = "red", linewidth = 1) +
      geom_point(aes(y = total_price / scale_factor), color = "red", size = 2) +
