                          ...
                          fluidRow(
                            ...
                            column(6,
                                   ...
                                   tags$div(
                                     ...
                                     tags$iframe(
                                       src = "https://www.youtube.com/embed/z2cgNGphQS0",
                          ...
                          fluidRow(
                            column(8,
                                 ...
                                 "
                                  ...
                                  <p>at the <a href="https://www.evansville.edu/">University of Evansville</a>, led by <br> <b>Dr. Omid Khormali</b>.<p>'
                                       ),
                          ...
                          fluidRow( column(3,),
                                    column(9,
                                           sliderInput(inputId = "dates",
                                                       ...
                                                       min = as.Date('2021-06-01'),#as.Date(min(monthly_sequence)),
                                                       max = as.Date('2024-08-01'),#as.Date(max(monthly_sequence)),
...
server <- function(input, output, session) {
  ...
  observe({
    req(potral_datasets()$monthly_sequence)  # Ensure `dates()` is available
    
    monthly_sequence = potral_datasets()$monthly_sequence
  ...
  ##########################################################
  # Observe map clicks for selecting multiple counties
  # 
  # selected_counties <- reactiveVal(character(0))
  # 
  # 
  # observeEvent(input$map_pop_shape_click, {
  #   click <- input$map_pop_shape_click  # Capture the click event
  #   clicked_county <- click$id      # Extract the clicked county name
  #   
  #   # Update the list of selected counties
  #   selected <- selected_counties()
  #   if (clicked_county %in% selected) {
  #     # If already selected, remove it
  #     selected <- setdiff(selected, clicked_county)
  #   } else {
  #     # Otherwise, add it
  #     selected <- c(selected, clicked_county)
  #   }
  ...
  #       layerId = ~County,
  #       label = ~County,
  #       color = ~ifelse(County %in% selected, "orange", "gray"),
  #       weight = 1,
  #       fillOpacity = 0.5
  #     )
  # })
  # 
  # # Clear selection when the button is clicked
  # observeEvent(input$clear_selection, {
  #   selected_counties(character(0))  # Reset selected counties
  #   
